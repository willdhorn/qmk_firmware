...
#define ACTION_TAP_HOLD(kc_tap, kc_hold) \
  ACTION_TAP_HOLD_FULL(kc_tap, LSFT(kc_tap), kc_hold, LSFT(kc_hold))

#define ACTION_TAP_HOLD_FULL(kc_tap, kc_tap_shift, kc_hold, kc_hold_shift) \
  { kc_tap, kc_tap_shift, kc_hold, kc_hold_shift, 0, th_default }

#define ACTION_TAP_CMD_HOLD(kc_tap) \
  ACTION_TAP_HOLD_FULL(kc_tap, LSFT(kc_tap), LCMD(kc_tap), LSFT(LCMD(kc_tap)))
//clang-format on
typedef enum {
  th_default,
  th_tapped,
